...
含义：8 行对应 8 个方向；每行 6 帧，从左到右循环播放。运行时按固定 `128x128` 单元格读取，不再按透明边界裁切。
...
如果后续确实需要更省内存的占位素材，可以临时降为 `8 行 x 2 列`，但正式人物和怪兽素材按 `8 行 x 6 列` 交付，并必须仍然保持 `128x128` 单格和同样的行顺序。
